...
fun RouteChatView(
    ...
    Canvas(
        ...
        points.forEach {
            val mX = (it.route % 7 + 1) * itemWidth + itemWidth / 2
            val mY = it.route / 7 * itemHeight * 2 + itemHeight / 2 + tSize / 2
            val x = (it.route % 7 + 1) * itemWidth + itemWidth / 2
            val y = it.route / 7 * itemHeight * 2 + itemHeight / 2 * 3 + tSize / 2
            drawIntoCanvas { canvas ->
                canvas.nativeCanvas.drawText(
                    String.format("%03d", it.route),
                    mX,
                    mY,
                    paint.apply {
                        textSize = tSize
